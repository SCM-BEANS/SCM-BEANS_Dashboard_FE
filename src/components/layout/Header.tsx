    ...
    <header className="w-full bg-transparent flex justify-between items-center h-16 px-4 md:px-6 z-30">
      ...
      <div className="flex items-center gap-3 md:gap-4">
        ...
        {isLoggedIn ? (
            ...
            {isDropdownOpen && (
              <div className="absolute right-0 mt-2 w-56 bg-surface border border-outline rounded-xl shadow-xl overflow-hidden z-50">
                <div className="px-4 py-3 border-b border-outline">
                  <p className="text-sm font-semibold text-on-surface truncate">{username || "User"}</p>
                  <p className="text-xs text-on-surface-variant truncate">Logged In</p>
                </div>
                <div className="py-1">
          ...
          <Link href="/login" className="text-primary font-mono text-xs md:text-sm uppercase font-semibold px-3 py-1.5 rounded-md border border-primary hover:bg-primary/10 transition-colors">
            Login
          </Link>
        )}
      </div>
