...
export function InnovationSection() {
  return (
    <section id="technology" className="bg-[#1A0E07] py-32 px-6 overflow-hidden">
      <div className="max-w-[1400px] mx-auto grid grid-cols-1 lg:grid-cols-2 gap-20 items-center">
        ...
        <div className="text-left flex flex-col items-start relative z-10">
          ...
          <p className="text-white/85 text-lg font-light mb-10 max-w-lg leading-relaxed">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-12 w-full">
            {features.map((f, i) => (
              <div key={i} className="flex flex-col gap-2.5 bg-white/10 hover:bg-white/15 transition-all p-5 rounded-xl border border-white/15">
                <div className="text-white/80">{f.icon}</div>
                <div className="text-white font-bold text-sm">{f.title}</div>
                <div className="text-white/80 text-sm font-light leading-relaxed">{f.desc}</div>
              </div>
            ))}
          </div>
          ...
          <Link
            ...
            className="group inline-flex items-center gap-3 px-8 py-4 border border-white/40 text-white rounded-full font-bold uppercase tracking-widest text-sm hover:bg-white hover:text-[#1A0E07] transition-all duration-300"
          >
        ...
        <div className="relative w-full h-[500px] lg:h-[650px] flex items-center justify-center">
          {/* Decorative rings */}
          <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[90%] aspect-square border border-white/8 rounded-full animate-[spin_60s_linear_infinite]" />
          <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[60%] aspect-square border border-white/10 rounded-full animate-[spin_40s_linear_infinite_reverse]" />

          {/* Dashboard mockup */}
          <div className="relative z-10 w-full max-w-[440px] bg-[#2C1208] border border-white/15 rounded-2xl p-5 shadow-[0_0_60px_rgba(0,0,0,0.5)]">
            <div className="flex items-center gap-2 mb-4">
              <span className="w-3 h-3 rounded-full bg-red-500/80" />
              <span className="w-3 h-3 rounded-full bg-yellow-500/80" />
              <span className="w-3 h-3 rounded-full bg-green-500/80" />
              <span className="text-white/60 text-xs ml-2 tracking-widest">DEER COFFEE · DASHBOARD</span>
            </div>

            <div className="grid grid-cols-3 gap-3 mb-4">
              {[
                { label: "Machines Online", value: "23 / 25", color: "text-green-400" },
                { label: "Today's Brews", value: "1,247", color: "text-amber-400" },
                { label: "Alerts", value: "2", color: "text-orange-400" },
              ].map((stat, i) => (
                <div key={i} className="bg-white/8 border border-white/10 rounded-lg p-3">
                  <div className={`text-xl font-black ${stat.color}`}>{stat.value}</div>
                  <div className="text-white/65 text-[10px] mt-1 tracking-wider uppercase">{stat.label}</div>
                </div>
              ))}
            </div>
            ...
            <div className="bg-white/8 border border-white/10 rounded-xl p-4 mb-4">
              <div className="text-white/65 text-[10px] tracking-widest uppercase mb-3">Hourly Extraction Volume</div>
              <div className="flex items-end gap-1.5 h-16">
                {[30, 55, 45, 70, 90, 65, 80, 75, 55, 85, 60, 45].map((h, i) => (
                  <div key={i} className="flex-1 bg-amber-500/50 rounded-sm" style={{ height: `${h}%` }} />
                ))}
              </div>
            </div>
            ...
            <div className="flex flex-col gap-2">
              {[
                ...
                <div key={i} className="flex items-center justify-between bg-white/8 border border-white/10 px-3 py-2 rounded-lg">
